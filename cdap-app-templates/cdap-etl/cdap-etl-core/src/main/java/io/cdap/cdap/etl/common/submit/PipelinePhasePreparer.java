  ...
  public List<Finisher> prepare(PhaseSpec phaseSpec)
    ...
    for (String stageName : phase.getDag().getTopologicalOrder()) {
      ...
      if (BatchSource.PLUGIN_TYPE.equals(pluginType) || isConnectorSource) {
        ...
        //And remember which runtime implementation agreed to prepareRun and use same implementation later
        SubmitterLifecycle<?> plugin = pluginInstantiator.newPluginInstance(stageName, macroEvaluator);
        if (plugin instanceof Transform) {
          Transform<?, ?> transform = (Transform<?, ?>) plugin;
          submitterPlugin = createTransform(transform, stageSpec);
        } else  {
          submitterPlugin = create(stageSpec, plugin);
